...
I chose this idea because tarot readings are easy for a language model to fake, but the actual card draw should not be invented by the model. A reusable skill makes the workflow more reliable because the Python script handles the random and deterministic parts of the process.
...
The script in `.agents/skills/reflective-tarot-draw/scripts/tarot_draw.py` does the load-bearing part of the workflow. It:
